...
// Mock function to simulate fetching top 10 places to visit
function fetchTopPlaces(country) {
    return new Promise((resolve) => {
        const mockPlaces = {
            ...
            // Add more countries and places as needed
        };
        setTimeout(() => resolve(mockPlaces[country] || []), 1000); // Simulate API call delay
    });
}
    ...
    if (!spinning) {
        ...
        fetchTopPlaces(selectedCountry).then(places => {
            if (places.length > 0) {
                ...
                // result.innerHTML += `<br>No places found to visit in ${selectedCountry}.`;
                result.innerHTML += '';
            }
        });
    }
